...
const Gallery = React.memo(({ photos, margin = 2, limitNodeSearch = 2, targetRowHeight = 300 }: Props) => {
    ...
    const images = computeRowLayout({ containerWidth: width, limitNodeSearch, targetRowHeight, margin, photos });

    return (
        <GalleryContainer>
            <GalleryComponent ref={galleryItem} >
                {images.map((thumb: Photo, index: number) => {
                    return <Photo key={index} photo={thumb} margin={margin} />
                })}
            </GalleryComponent>
        </GalleryContainer>
    );
